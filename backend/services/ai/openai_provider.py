...
class OpenAIProvider(BaseAIProvider):
    ...
    description: str = f"OpenAI Cloud API ({settings.OPENAI_MODEL})"
    ...
    def is_available(self) -> bool:
        """Returns True if OpenAI API key is configured and not exhausted."""
        has_key = bool(settings.OPENAI_API_KEY and settings.OPENAI_API_KEY.strip() and not settings.OPENAI_API_KEY.startswith("sk-placeholder"))
        return has_key and not self._quota_exhausted
    ...
    def _generate_fallback_response(self, message: str, intent: UserIntent, products: List[RecommendedProduct]) -> str:
        ...
        summary_lines = [
            f"Based on your requirements{budget_str}{use_case_str}, here are **{len(products)} top-rated recommendations**:",
            "",
